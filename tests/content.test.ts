...
describe('CVEHighlighter Content Script', () => {
  ...
  describe('DOM Highlighting', () => {
    ...
    it('should handle multiple CVEs in the same text node', () => {
      const text =
        'Multiple CVEs: CVE-2023-1111, CVE-2023-2222, and CVE-2023-3333.';
    ...
    it('should identify excluded elements correctly', () => {
      ...
      const elements = document.querySelectorAll(
        'script, style, noscript, div'
      );

      const excludedElements = Array.from(elements).filter((el) =>
        excludedTags.includes(el.tagName)
      );

      const eligibleElements = Array.from(elements).filter(
        (el) =>
          !excludedTags.includes(el.tagName) && el.textContent?.includes('CVE-')
      );
  ...
  describe('Message Handling', () => {
    it('should handle toggle message', async () => {
      const messageHandler = (
        mockChrome.runtime.onMessage.addListener as jest.Mock
      ).mock.calls[0]?.[0];
      ...
      if (messageHandler) {
        ...
        // Test enabling
        (mockChrome.storage.local.get as jest.Mock).mockImplementationOnce(
          (keys, callback) => {
            if (callback) {
        ...
        expect(mockChrome.storage.local.set as jest.Mock).toHaveBeenCalledWith(
  ...
  describe('Tooltip Display', () => {
    it('should create tooltip element', () => {
      ...
      expect(tooltipElement?.classList.contains('vulncheck-tooltip')).toBe(
        true
      );
    });
  ...
  describe('MutationObserver Integration', () => {
    it('should observe DOM mutations', () => {
      ...
      expect((observer as any).observe).toHaveBeenCalledWith(
        document.body,
